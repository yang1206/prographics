  ...
  public:
    ...
    bool updateRange(const std::vector<float>& newData) {
        ...
        if (!m_rangeInitialized) {
            ...
            auto [initialMin, initialMax] = calculateNiceRange(newMin - buffer, newMax + buffer, m_config.tickCount);
        ...
        if (++m_resetCounter % 50 == 0 || forceUpdate) {
            ...
            // 检查显示范围是否发生显著变化
            bool rangeChanged =
                isRangeChangedSignificantly(oldDisplayMin, oldDisplayMax, newDisplayMin, newDisplayMax, forceUpdate);
